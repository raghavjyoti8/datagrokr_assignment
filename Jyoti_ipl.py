...
df = spark.read.csv('/FileStore/tables/ipl_ball_by_ball.csv',header=True)
df1 = spark.read.csv('/FileStore/tables/ipl_matches.csv',header=True)
df2 = spark.read.csv('/FileStore/tables/ipl_venue.csv',header=True)
...
#Doing  the Analysis on the ipl dataset
...
# here if I see which teams has won the tosses maximum times then I came to know that Mumbai Indian won 106 times and kolkata knight 98 times and like so on, but when I see the data like Which team has won the toss most number of times in IPL the the outcomes are Mumbai Indians 120 matches and chennai super kings  106 matches plus teams who won with biggest margins are also Mumbai Indians and teams like chennai super kings .
#team has won the toss most number of times in IPL
